...
final class TopupConfirmViewController: UIViewController {
    ...
    private func makeInfoCard() -> UIView {
        let stack = UIStackView(arrangedSubviews: [
            ...
            makeRow(title: "Phương thức", value: "VNPay"),
    ...
    private func setupPinSheet() {
        ...
        let confirmButton = makePrimaryButton(title: "Mở VNPay")
        confirmButton.addTarget(self, action: #selector(tapSubmitPin), for: .touchUpInside)

        activity.hidesWhenStopped = true

        let stack = UIStackView(arrangedSubviews: [titleLabel, subtitleLabel, pinField, confirmButton, activity])
        stack.axis = .vertical
        stack.spacing = 16
        stack.translatesAutoresizingMaskIntoConstraints = false
